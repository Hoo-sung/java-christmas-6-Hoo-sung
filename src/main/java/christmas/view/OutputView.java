...
public final class OutputView {
    ...
    public static void printOriginalTotalAmount(int originalTotalAmount){
        ...
        printMessage(Util.createFormattedAmount(originalTotalAmount)+"원");
        printEmptyLine();
    }
    ...
    public static void printBonusMenu(int originalTotalAmount){
        ...
        for(BonusMenu bonusMenu : BonusMenu.values()){
            printMessage(bonusMenu.getName()+" "+Util.createFormattedAmount(bonusMenu.getQuantity())+"개");
        }
        printEmptyLine();
    }
    ...
    public static void printDiscountRecord(DiscountRecord discountRecord){
        ...
        if(discountRecord.getdDayDiscountAmount() !=0 )
            printMessage("크리크마스 디데이 할인: -"+Util.createFormattedAmount(discountRecord.getdDayDiscountAmount())+"원");
        if(discountRecord.getWeekdayDiscountAmount()!=0)
            printMessage("평일 할인: -"+Util.createFormattedAmount(discountRecord.getWeekdayDiscountAmount())+"원");
        if(discountRecord.getWeekendDiscountAmount()!=0)
            printMessage("주말 할인: -"+Util.createFormattedAmount(discountRecord.getWeekendDiscountAmount())+"원");
        if(discountRecord.getStarDayDiscountAmount()!=0)
            printMessage("특별 할인: -"+Util.createFormattedAmount(discountRecord.getStarDayDiscountAmount())+"원");
        if(discountRecord.getBonusEventDiscount()!=0)
            printMessage("증정 이벤트: -"+Util.createFormattedAmount(discountRecord.getBonusEventDiscount())+"원");
        printEmptyLine();
    }
    ...
    public  static void printTotalDiscountAmount(DiscountRecord discountRecord){
        ...
        printMessage("-"+Util.createFormattedAmount(discountRecord.getTotalDiscountAmount())+"원");
        printEmptyLine();
    }

    public static void printExpectedPayment(int expectedPayment){
        printMessage("<할인 후 예상 결제 금액>");
        printMessage(Util.createFormattedAmount(expectedPayment)+"원");
        printEmptyLine();
    }
